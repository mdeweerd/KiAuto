...
PCB_DIFF=${PCB_DIFF:=${TOOL_PATH}/pcb-diff.sh}
LOGFILE=/dev/null
LOGFILE=$0.log
...
echo "# ${PCB_DIFF} $PREV_TEMP_PATH" "$CUR_TEMP_PATH" $WORKDIR >> $LOGFILE
${PCB_DIFF}  "$PREV_TEMP_PATH" "$CUR_TEMP_PATH" $WORKDIR >> $LOGFILE 2>&1

echo -n $WORKDIR/brd/montage.png
